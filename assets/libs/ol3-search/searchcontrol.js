...
ol.control.Search = function (options) {
  ...
  if (options.target) {
    ...
    this.button = $("<button>").attr('type', 'button').click(function () {
  ...
  // Search input
  var tout, cur = "";
  $("<input>").attr('type', 'search').addClass("search").attr('placeholder', options.placeholder || "Search...").on('change', function (e) {
    self.dispatchEvent({
      type: "change:input",
      input: e,
      value: $(this).val()
    ...
    // Search / autocomplete
    else if ((e.type == "search" || e.key == 'Enter') || (cur != val && options.typing >= 0)) { // current search
      cur = val;
      if (cur) { // prevent searching on each typing
...
ol.control.Search.prototype.drawList_ = function (auto) {
  ...
  for (var i = 0; i < max; i++) {
    $("<li>").html(self.getTitle(auto[i])).data('search', auto[i]).click(function (e) {
